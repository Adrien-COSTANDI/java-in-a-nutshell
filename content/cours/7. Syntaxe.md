...
On notera aussi les conventions de nommage. 
Java adopte deux conventions selon le cas :
- `camelCase` pour nommer à peu près tout ce qui doit avoir un nom
- `PascalCase` pour nommer les fichiers et les [[8. Variables#Types|types objets]] (qu'on verra juste après)

La convention `camelCase` consiste commencer par une minuscule puis à ajouter une majuscule à chaque nouveau mot pour améliorer la lisibilité puisqu'il n'est pas possible de les séparer par des espaces.
`voilaUnExempleDePhraseEnCamelCase`.

La convention `PascalCase` fait exactement la même chose mais en commençant par une majuscule.
`VoilaUnExempleDePhraseEnPascalCase`.
