...
À la ligne 3, je déclare une variable et je l'initialise avec la valeur de retour du `switch`. Selon le cas, le mot clé `yield` va me renvoyer le bon texte dans `resultat`. Comme la valeur de `variable` est 20, le cas `20, 30` sera exécuté et mon `resultat` vaudra `"texte 2"` !

Si on exécute ce programme en le mettant dans un `main`, on obtiendra dans la console :
```
variable vaut 20 ou 30
resultat = texte 2
```

Comme d'habitude, il a aussi le droit à une syntaxe simplifiée quand les blocs de code ne comprennent qu'une seule ligne en retirant les accolades :
...
IO.println("resultat = " + resultat);
```
> *Le `yield` n'est plus nécessaire s'il n'y a qu'une seule ligne !*
