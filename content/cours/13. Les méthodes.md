...
Dans ma méthode `isEven`, j'ai deux fins alternatives : soit le nombre est pair, soit il ne l'est pas. Suivant quelle branche du `{java} if` est exécutée, je renverrai `true` ou `false` mais je ne verrai jamais `Coucou !` s'afficher dans la console !
...
void main() {
  ...
  while(!gameOver) { // tant que la partie n'est pas finie
    ...
    gameOver = checkGameOver(gameState); // on vérifie si la partie est finie après ce tour
  }
}
```
...
Dans ces cas là, il est essentiel de savoir manipuler des objets. On rentre dans le vrai morceau ! [[16. Les objets (Partie 1)|YEEEHAAAA !]]
