...
- les 8 types primitifs (en minuscule) :
    ...
    - `{java} float`
    - `{java} double`(**\***)
(**\***) : ces types sont importants, retenez les en priorité
...
L'initialisation d'une variable consiste à lui donner une valeur. On fait ça avec l'**opérateur d'assignation** `=`.
...
```java
int a = 6; // une ligne au lieu de deux !
```
...
![[compilerInference.png]]
> Je passe mon curseur de souris sur la variable `c`.

Maintenant qu'on a des variables, on va pouvoir les manipuler avec des [[9. Opérateurs|opérateurs]] !